...
namespace PopForums.Data.SqlSingleWebServer.Repositories
{
	public class ForumRepository : IForumRepository
	{
		...
		{
			Forum forum = null;
			_sqlObjectFactory.GetConnection().Using(connection => connection.Command("SELECT " + _forumFields + " FROM pf_Forum WHERE ForumID = @ForumID")
		...
		public Forum Get(string urlName)
		{
			...
			_sqlObjectFactory.GetConnection().Using(connection => 
				connection.Command("SELECT " + _forumFields + " FROM pf_Forum WHERE UrlName = @UrlName")
		...
		public List<Forum> GetForumsInCategory(int? categoryID)
		{
			...
			if (categoryID.HasValue && categoryID != 0)
				_sqlObjectFactory.GetConnection().Using(connection =>
					connection.Command("SELECT " + _forumFields + " FROM pf_Forum WHERE CategoryID = @CategoryID")
						.AddParameter("@CategoryID", categoryID.Value)
						.ExecuteReader()
						.ReadAll(r => forums.Add(PopulateForumFromReader(r))));
			else
				_sqlObjectFactory.GetConnection().Using(connection =>
					connection.Command("SELECT " + _forumFields + " FROM pf_Forum WHERE CategoryID = 0 OR CategoryID IS NULL")
						.ExecuteReader()
						.ReadAll(r => forums.Add(PopulateForumFromReader(r))));
			return forums;
		}
		...
		public IEnumerable<Forum> GetAll()
		{
			var forums = new List<Forum>();
			_sqlObjectFactory.GetConnection().Using(connection => 
				connection.Command("SELECT " + _forumFields + " FROM pf_Forum ORDER BY SortOrder")
			        .ExecuteReader()
			        .ReadAll(r => forums.Add(PopulateForumFromReader(r))));
			return forums;
		}
		...
		public IEnumerable<Forum> GetAllVisible()
		{
			var forums = new List<Forum>();
			_sqlObjectFactory.GetConnection().Using(connection =>
				connection.Command("SELECT " + _forumFields + " FROM pf_Forum WHERE IsVisible = 1 ORDER BY SortOrder")
					.ExecuteReader()
					.ReadAll(r => forums.Add(PopulateForumFromReader(r))));
			return forums;
		}
